# Beyond Birthday sprites

Copy these files from your Shimeji pack into this folder:

**Phase 1 (idle + walk):**
- `shime1.png` — idle / standing
- `shime2.png` — walk frame
- `shime3.png` — walk frame

**Drag + fall animations:**
- `shime4.png` — falling
- `shime5.png`, `shime6.png` — resist (grab struggle)
- `shime7.png` through `shime10.png` — grabbed lean poses
- `shime18.png`, `shime19.png` — landing bounce

You can add `shime11.png` through `shime46.png` later for more animations.

The app loads sprites from `/companion/beyond-birthday/` at runtime.
